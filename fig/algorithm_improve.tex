...
\usepackage[paperheight=19.3cm, paperwidth=5.5in, margin=0mm]{geometry}
\usepackage{amsmath}
\usepackage{amssymb}
...
\begin{algorithm}
  ...
  \KwIn{Two-dimensional flow direction ($\mathbb{F}$), drainage area ($\mathbb{A}$), upstream accumulation length ($\mathbb{L}$), critical drainage area ($A_{\mathrm{cr}}$), and critical length ($l_{\mathrm{cr}}$)}
  ...
  \While{\normalfont{$Open$ is not empty}}{
    ...
    \Loop{}{
      ...
      \If{$\mathbb{A}(i_2, j_2) \leq A_{\mathrm{cr}}$}{
        $Lines$.append($L$)\;
        break\;
      }
